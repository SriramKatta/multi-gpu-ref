...
int main(int argc, char *argv[])
{
  ...
  nvtxRangePushA("Graph_creation");
  {
    ...
    for (size_t i = 0; i < 2; i++)
    {
      ...
      CUDA_CALL(cudaEventRecord(inner_done, inner_stream));
      ...
      CUDA_CALL(cudaStreamEndCapture(inner_stream, graphs + i));
      std::swap(a, a_new);
    }
    {
      CUDA_CALL(cudaGraphInstantiateWithFlags(&graph_exec[0], graphs[0], cudaGraphInstantiateFlagUseNodePriority));
      CUDA_CALL(cudaGraphInstantiateWithFlags(&graph_exec[1], graphs[1], cudaGraphInstantiateFlagUseNodePriority));
    }
    // cudaGraph_t objects are no longer required after instantiation
    CUDA_CALL(cudaGraphDestroy(graphs[0]));
    CUDA_CALL(cudaGraphDestroy(graphs[1]));
  }
  nvtxRangePop();
  ...
  for (int i = 0; i < 10; ++i)
  {
    CUDA_CALL(cudaGraphUpload(graph_exec[0], inner_stream));
    CUDA_CALL(cudaGraphUpload(graph_exec[1], inner_stream));
